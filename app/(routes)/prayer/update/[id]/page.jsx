...
export default function PrayerUpdatePage({ params }) {
  ...
  // Existing S3 URLs from DB
  const [existingPhotoUrl, setExistingPhotoUrl] = useState("");
  const [existingAudioUrl, setExistingAudioUrl] = useState("");

  // New local files chosen by user
  ...
  useEffect(() => {
    ...
    setId(recordId);
    GlobalApi.prayerById({ id: recordId })
      .then((resp) => {
        const prayer = resp.data.data;
        setTitle(prayer.title || "");
        setContent(prayer.content || "");
        setExistingPhotoUrl(prayer.url || "");
        setExistingAudioUrl(prayer.audioUrl || "");
        setSelectedType(prayer.type?._id || prayer.type || "");
        setSelectedLanguage(prayer.language?._id || prayer.language || "");
      })
      .catch(() => toast.error("Failed to load prayer."))
  ...
  const handleUpdate = () => {
    ...
    setIsLoading(true);
    GlobalApi.updatePrayer(formData)
      .then((result) => {
  ...
  // What to show in the photo zone
  const displayPhoto = photoPreview || existingPhotoUrl;

  return (
    <>
      <Head><title>Update Prayer</title></Head>
      ...
      <style>{`
        ...
        .cp-photo-preview-wrap { position: relative; width: 100%; height: 150px; }
        ...
        .cp-audio-existing {
          ...
          padding: 11px 13px;
          background: var(--accent-light);
          border: 1px solid rgba(124,77,47,0.25);
          border-radius: var(--radius);
          margin-bottom: 10px;
        }
        .cp-audio-existing svg { color: var(--accent); flex-shrink: 0; width: 16px; height: 16px; }
        .cp-audio-existing-info { flex: 1; min-width: 0; }
        .cp-audio-existing-label { font-size: 10px; font-weight: 700; letter-spacing: 0.07em; text-transform: uppercase; color: var(--accent); }
        .cp-audio-existing-name { font-size: 12px; color: var(--ink-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      ...
      {/* Hidden file inputs — display:none prevents double-open */}
      <input
        type="file"
        accept="image/*"
        style={{ display: "none" }}
      ...
      <div className="cp-root">
        <div className="cp-layout">
          ...
          <div className="cp-nav">
            ...
            <span className="cp-badge">Edit Prayer</span>
          </div>

          {/* ── Main ── */}
          <div className="cp-main">
            <div className="cp-main-header">
              <h1 className="cp-heading">Update Prayer</h1>
              <p className="cp-subheading">Edit and save changes to this prayer</p>
            </div>
            ...
            <div className="cp-main-body">
              ...
              <div className="cp-field">
                ...
                {isFetching
                  ? <div className="cp-skeleton" style={{ height: 42 }} />
                  : <input
                    className="cp-input"
                    value={title}
          ...
          <div className="cp-sidebar">
            ...
            <div className="cp-panel">
              ...
              <div className="cp-panel-body">
                {isFetching ? (
                  <div className="cp-skeleton" style={{ height: 60, borderRadius: 6 }} />
                ) : (
                  <>
                    {/* Show existing audio from DB if no new file chosen */}
                    {existingAudioUrl && !audioFile && (
                      <div className="cp-audio-existing">
                        <Music size={16} />
                        <div className="cp-audio-existing-info">
                          <div className="cp-audio-existing-label">Current audio</div>
                          <div className="cp-audio-existing-name">
                    ...
                    {audioFile ? (
                      <div className="cp-audio-success">
                        ...
                        <div className="cp-audio-success-info">
                          ...
                          <div className="cp-audio-success-sub">Will replace current audio</div>
                        </div>
                      ...
                      >
                        <div className="cp-audio-inner">
                          <UploadCloud />
                          <strong>{existingAudioUrl ? "Replace audio" : "Drop file or click to browse"}</strong>
            ...
            <div className="cp-panel">
              <div className="cp-panel-body" style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <p className="cp-hint">Only upload new files if you want to replace the existing ones.</p>
              </div>
            </div>
