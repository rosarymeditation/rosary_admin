...
import { Loader2, ChevronLeft, Music, X, UploadCloud, CheckCircle2 } from "lucide-react";
import { toast } from "sonner";

export default function PsalmUpdatePage({ params }) {
  ...
  useEffect(() => {
    ...
    setId(recordId);
    GlobalApi.psalmById({ id: recordId })
      .then((resp) => {
        const psalm = resp.data.data;
        setTitle(psalm.title || "");
        setContent(psalm.content || "");
        setVerse(psalm.verse || "");
        setSelectedLanguage(psalm.language?._id || psalm.language || "");
        setExistingAudioUrl(psalm.audioUrl || "");
      })
      .catch(() => toast.error("Failed to load psalm."))
  ...
  const handleUpdate = () => {
    ...
    setIsLoading(true);
    GlobalApi.psalmUpdate(formData)
      .then((result) => {
        if (!result.data.error) {
          toast.success("Psalm updated successfully.");
          router.push("/psalm/list");
        } else {
  ...
  const isValid = title.trim() && verse.trim() && content.trim() && selectedLanguage;

  return (
    <>
      <Head><title>Update Psalm</title></Head>
      ...
      <div className="cp-root">
        <div className="cp-layout">
          ...
          <div className="cp-nav">
            <Link href="/psalm/list" className="cp-back">
              <ChevronLeft size={14} /> All Psalms
            </Link>
            <span className="cp-badge">Edit Psalm</span>
          </div>

          {/* ── Main ── */}
          <div className="cp-main">
            <div className="cp-main-header">
              <h1 className="cp-heading">Update Psalm</h1>
              <p className="cp-subheading">Edit and save changes to this psalm</p>
            </div>
            ...
            <div className="cp-main-body">
              ...
              <div className="cp-row">
                ...
                <div className="cp-field">
                  <label className="cp-label">Verse</label>
                  {isFetching
                    ? <div className="cp-skeleton" style={{ height: 42 }} />
                    : <input
                      className="cp-input"
              ...
              <div className="cp-field">
                ...
                {isFetching
                  ...
                  : <textarea
                    ...
                    placeholder="Write the psalm text here..."
                  />
                }
              </div>
          ...
          <div className="cp-sidebar">
                    ...
                    {existingAudioUrl && !audioFile && (
                      <div className="cp-audio-existing">
                        ...
                        <div className="cp-audio-existing-info">
                          ...
                          <div className="cp-audio-existing-name">{existingAudioUrl.split("/").pop()}</div>
                        </div>
                      </div>
                    )}
            ...
            <div className="cp-panel">
              <div className="cp-panel-body" style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <p className="cp-hint">Only upload a new file if you want to replace the existing audio.</p>
              </div>
            </div>
