...
export default function PsalmList() {
  ...
  useEffect(() => {
    GlobalApi.psalmList()
      .then((resp) => setPsalms(resp.data.data))
      .catch(() => toast.error("Failed to load psalms."))
      .finally(() => setIsFetching(false));
  }, []);

  const handleDelete = (id) => {
    if (!confirm("Are you sure you want to delete this psalm?")) return;
    setDeletingId(id);
    GlobalApi.deletePsalm(id)
      .then((result) => {
        if (!result.data.error) {
          setPsalms((prev) => prev.filter((p) => p._id !== id));
          toast.success("Psalm deleted.");
        } else {
          toast.error("Failed to delete psalm.");
        }
      })
  ...
  return (
    <>
      <Head><title>Psalm List</title></Head>
      ...
      <style>{`
        ...
        .pl-inner { max-width: 860px; margin: 0 auto; }
          ...
          <div className="pl-header">
            <div>
              <h1 className="pl-heading">
                Psalms
                {!isFetching && <span className="pl-count">{psalms.length}</span>}
              </h1>
              <p className="pl-subheading">Manage your psalm collection</p>
            </div>
            <Link href="/psalm/create" className="pl-add-btn">
              <PlusIcon /> Add Psalm
            </Link>
          </div>
            ...
            <table className="pl-table">
              <thead className="pl-thead">
                <tr>
                  ...
                  <th className="pl-th">Verse</th>
                  <th className="pl-th">Language</th>
              ...
              <tbody>
                ...
                {isFetching && Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i} className="pl-tr">
                    <td className="pl-td"><div className="pl-skeleton" style={{ width: "55%" }} /></td>
                ...
                {/* Empty */}
                {!isFetching && psalms.length === 0 && (
                  <tr>
                    <td colSpan={5}>
                      <div className="pl-empty">
                        <div className="pl-empty-icon"><PlusIcon size={22} /></div>
                        <strong>No psalms yet</strong>
                        <span>Click "Add Psalm" to create the first one.</span>
                      </div>
                    </td>
                  </tr>
                )}
                ...
                {/* Rows */}
                {!isFetching && psalms.map((item) => (
                  <tr key={item._id} className="pl-tr">
                    <td className="pl-td">
                      <span className="pl-title">{item.title}</span>
                    </td>
                    <td className="pl-td">
                      <span className="pl-verse">{item.verse}</span>
                    </td>
                    <td className="pl-td">
                      <span className="pl-lang">{item.language?.name || "—"}</span>
                    </td>
                    <td className="pl-td center">
                      {item.audioUrl
                        ? <span className="pl-audio-icon" title="Has audio"><Music2 /></span>
                        : <span className="pl-no-audio">—</span>
                      }
                    </td>
                    <td className="pl-td right">
                      <div className="pl-actions">
                        <Link href={`/psalm/update/${item._id}`} className="pl-action-btn edit">
                          <EditIcon />
                        </Link>
                        <button
